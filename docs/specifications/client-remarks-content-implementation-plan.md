...
The client document is a working Russian approval version. It is authoritative for the requested positioning and for values explicitly labelled as corrections, but it also marks several facts as requiring confirmation. Those conditional facts are not published as confirmed claims. On 2026-09-07 the owner separately approved the InnoTrans primary-navigation destination, dedicated page, partner logos, and map; that approval opens only the source-bounded event slice recorded below.
...
Approved InnoTrans slice — 2026-09-07:

- add `InnoTrans 2026` as a localized top-level primary-navigation item and publish `/innotrans-2026/` plus localized equivalents;
- publish the client-supplied invitation, 22–25 September 2026 dates, Messe Berlin Outdoor Display location, and display positions T5/50, T5/55, T5/60, and O5/55;
- show only TAS Group and TAS Dniprovagonmash as approved partner identities, using locally served logo artwork recovered from official company sources with provenance recorded;
- create a TransAnt-authored simplified schematic for the four approved positions. Do not copy or republish the official InnoTrans grounds-plan artwork; link to the official current plan instead;
- link to ordinary directions for Messe Berlin and the official TransAnt InnoTrans profile;
- use the existing contact/mail-client handoff with event context for meeting requests. Do not create a new meeting form, server endpoint, calendar integration, or provider;
- omit the exhibit list until the final list is confirmed. Do not infer which wagon occupies which approved position.
...
| Top-level pages                | Add Projects and InnoTrans 2026 to the main menu                                                                             | Projects exists; InnoTrans currently appears as a homepage announcement only                                                                       | **InnoTrans approved 2026-09-07:** add the localized top-level destination and dedicated `/innotrans-2026/` route; Projects remains unchanged                                                                                                 |
...
| InnoTrans page                 | Add stands, exhibits, partners, custom site map, Google Maps route, and meeting form                                         | Dates/location and official profile link are published; client page 8 supplies four display positions and names TAS Group plus TAS Dniprovagonmash | **Approved source-bounded slice 2026-09-07:** publish dates/location/positions and two approved partners, use a locally drawn schematic plus official-plan/directions links, reuse the current contact handoff, and omit unconfirmed exhibits |
