...
- `sections` is a required non-empty ordered list. Each section has one H2 and
  one-or-more non-empty paragraphs. A paragraph may optionally append one
  caller-owned, labelled HTTPS legal reference.
...
overflow. Reference links have visible keyboard focus. The contract rejects
blank headings, paragraphs, introductions, and non-HTTPS references.
