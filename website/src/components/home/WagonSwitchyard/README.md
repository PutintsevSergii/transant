...
are caller-owned (`selectionLabel`, `selectionAriaLabel`, and
`supportingCopy`). They are part of the route content model rather than
component-owned English, so locale adapters can translate the whole blue
...
Each family supplies `id`, `sequence`, `familyName`, `modelCode`, `headline`, `summary`, imported local `image`, `href`, `linkLabel`, and optional non-empty `technicalLabel`. An unusually tall or wide render may opt into the caller-owned `mediaScale: "reduced"` stage treatment; it is proportionally scaled to preserve its aspect ratio while clearing the surrounding stage. The component otherwise preserves its standard contained scale. The public contract rejects unsafe or placeholder links, duplicate identities, remote/zero-size image metadata, invalid media-scale values, blank required copy, and counts other than five.
...
On desktop fine pointers, after the section is at least 70% visible, a non-essential preview advances the selected family every five seconds and loops through all five families while eligible. Hover, focus, and manual selection do not interrupt it; a manual selection merely establishes the next item in the sequence. It pauses only while the tab is hidden and is suppressed for reduced motion and data-saving. The preview has no control surface, never moves focus or scroll position, and has no live region, persistence, or telemetry.
...
At 320, 390, and 768 px, the section follows the documented source order: introduction, a native horizontal scroll-snap five-family rail with a partial next card, active identity/cargo fit, stable contained local wagon stage, summary, direct family action, and optional technical label. No compact selection or focus movement occurs automatically. At a 52-rem component width the rail becomes a fully visible five-column route track. At 64 rem the visual composition keeps the introduction left-aligned, places the large wagon stage on the left, the editorial detail panel on the right, and the selector below the framed result. Viewport-relative spacing and panel height keep the complete component within 1024×768 and 1440×900 while retaining the selector-first semantic and focus order. The stage remains contained with `object-fit: contain` at all widths.

The control-room chrome uses reader-facing guidance instead of internal status language: it asks visitors to choose by transport task and clarifies how cargo and loading needs map to the family pages. On the wide selector, the blue route line begins and ends at the outer stop centres, every stop is centred on its line, and sequence numbers sit below it. Twelve-pixel gutters keep the line visible between cards; the active red connector extends equally to the left and right of middle stops, without extending before the first stop or after the last. It does not copy unverified homologation or operating-readiness claims from the reference.
