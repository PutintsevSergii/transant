...
- Active objective: add InnoTrans 2026 to primary navigation and publish its dedicated localized event page with approved partner identities, logos, display positions, and a source-authored map
- Last completed work package: P-003
- Next eligible work package: A-002 — approved homepage Variant A follow-on
- Last change-log entry: 2026-09-07 — I-005 — InnoTrans 2026 page slice opened
...
## Active correction — I-005 InnoTrans 2026 navigation and page

- Outcome: `IN_PROGRESS`. The user approved the InnoTrans primary-navigation destination, dedicated event page, partner logos, and map. Client page 8 supplies dates, Messe Berlin Outdoor Display, positions T5/50, T5/55, T5/60, and O5/55, and the approved TAS Group and TAS Dniprovagonmash partner identities.
- Contract: add a localized `InnoTrans 2026` top-level navigation destination; publish canonical `/innotrans-2026/` plus DE/UA/PL/CZ equivalents; present the source invitation, dates, location, and four display positions; use only the two approved partner identities with locally served official-source logos; draw a local simplified stand schematic from approved positions rather than copy the official grounds plan; provide ordinary external links for Messe Berlin directions, the official venue plan, and the TransAnt exhibitor profile; and reuse the current contact/mail-client path for meeting requests. Withhold the unconfirmed exhibit list and do not introduce a meeting provider or new form workflow.
- Plan: `docs/specifications/client-remarks-content-implementation-plan.md`.
- Files: client plan and active page/responsive specifications; SiteHeader and localized shell adapters; InnoTrans page view model/layout/routes; locally served partner assets and provenance; page/component documentation; route/content audits; focused unit/browser/accessibility/responsive/visual coverage; tracker; `STATUS.md`; `CHANGELOG.md`.
...
- Current pre-edit baseline: `pnpm quality` reaches Astro diagnostics after Prettier and ESLint pass, then fails in the concurrent uncommitted `tests/unit/innotrans-page-view-model.test.ts:52` because `logo` narrows to `never` before `.startsWith()`. This is outside the catalogue-note correction; no baseline pass is claimed for the current working tree.
...
- Next exact action: recover official-source partner logo assets and event map references, implement the localized page and navigation contract, then run focused route/content/header/page/browser/accessibility/visual evidence before resynchronizing I-005.
